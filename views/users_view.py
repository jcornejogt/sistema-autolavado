...
class UsersView(ctk.CTkFrame):
    ...
    def __init__(self, master, usuario_actual):
        ...
        ctk.CTkLabel(formulario, text="Nombre completo").grid(
        ...
        self.usuario_entry = ctk.CTkEntry(formulario, width=250)
        self.usuario_entry.grid(row=1, column=1)

        ctk.CTkLabel(formulario, text="Contraseña").grid(
            row=2, column=0, padx=10, pady=10, sticky="w"
        )
        self.password_entry = ctk.CTkEntry(formulario, width=250, show="*")
        self.password_entry.grid(row=2, column=1)

        self.password_hint = ctk.CTkLabel(
            formulario, text="", font=("Arial", 11), text_color="gray"
        )
        self.password_hint.grid(row=3, column=1, sticky="w")

        ctk.CTkLabel(formulario, text="Rol").grid(
            row=4, column=0, padx=10, pady=10, sticky="w"
        )
        self.rol_combo = ctk.CTkComboBox(formulario, width=250, values=ROLES)
        ...
        self.btn_guardar = ctk.CTkButton(
            botones, text="Guardar", command=self.guardar_usuario
        )
    ...
    def guardar_usuario(self):
        ...
        if nombre == "" or usuario == "":
            messagebox.showwarning("Validación", "Ingrese nombre y usuario.")
            return
    ...
    def eliminar_usuario(self):
        ...
        if self.user_id_seleccionado is None:
            messagebox.showwarning("Usuario", "Seleccione un usuario.")
            return

        if not messagebox.askyesno("Eliminar", "¿Desea eliminar este usuario?"):
            return

        try:
            UserController.eliminar(
                self.user_id_seleccionado, self.usuario_actual["id"]
            )
        except Exception as e:
            messagebox.showerror("Error", str(e))
            return
    ...
    def cargar_usuarios(self):
        ...
        for user in UserController.listar():

            self.tabla.insert(
                "",
                "end",
                values=(user.id, user.nombre, user.usuario, user.rol)
            )